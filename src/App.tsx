...
function MapView({
  ...
  const groupNodes: FlowNode[] = groups.map((group, index) => ({
    id: `group-${group.id}`,
    type: 'topologyGroup',
    position: { x: 40 + (index % 3) * 280, y: 35 + Math.floor(index / 3) * 150 },
    data: { group },
    draggable: false,
    selectable: false,
  }));
