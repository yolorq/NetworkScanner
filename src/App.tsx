...
function MapView({
  ...
  type FlowData = { [key: string]: unknown; device: Device };
  type FlowNode = Node<FlowData> & { type: 'device' };
  ...
  const [groups, setGroups] = useState<TopologyGroup[]>(() => { try { return JSON.parse(localStorage.getItem('netscope-topology-groups') ?? '[]') as TopologyGroup[]; } catch { return []; } });
  ...
  const visibleNodes = activeGroup ? nodes.filter((node) => activeGroup.deviceIds.includes(node.id)) : nodes;
  ...
  const saveViewport=(next:Viewport)=>{setViewport(next);void onSaveMapView({x:next.x,y:next.y,zoom:next.zoom})}; const autoLayout=()=>void Promise.all(devices.map((device,index)=>onPosition(device.id,{position:{x:80+(index%4)*260,y:80+Math.floor(index/4)*155}})));
  ...
  const showContextMenu=(event:MouseEvent|React.MouseEvent,menu:{kind:'pane'}|{kind:'node';id:string}|{kind:'edge';id:string})=>{event.preventDefault();const bounds=mapCanvasRef.current?.getBoundingClientRect();if(!bounds)return;const flowPosition={x:(event.clientX-bounds.left-viewport.x)/viewport.zoom-110,y:(event.clientY-bounds.top-viewport.y)/viewport.zoom-40};setContextMenu({...menu,x:Math.min(event.clientX-bounds.left,bounds.width-220),y:Math.min(event.clientY-bounds.top,bounds.height-190),...(menu.kind==='pane'?{flowPosition}:{})} as ContextMenu)};
